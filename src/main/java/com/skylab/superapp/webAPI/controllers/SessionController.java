...
public class SessionController {
    ...
    public ResponseEntity<DataResult<SessionDto>> addSession(@RequestBody CreateSessionRequest createSessionRequest,
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new SuccessDataResult<>(result, SessionMessages.SESSION_CREATED_SUCCESSFULLY, HttpStatus.CREATED, request.getRequestURI()));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<DataResult<Void>> deleteSession(@PathVariable UUID id, HttpServletRequest request){
